...
abstract class AbstractSubscriber implements EventSubscriberInterface
{
    ...
    public function onSetInitialState(StateMachineEvent $event)
    {
        ...
        $previousState = $event->getStateMachine()->getCurrentState();
        ...
        $this->callCallback($object, $previousState, 'post', 'initialize');
    }
    ...
    public function onTestTransition(TransitionEvent $event)
    {
        $object = $event->getStateMachine()->getObject();
        $previousState = $event->getStateMachine()->getCurrentState();
        $result = $this->callCallback($object, $previousState, 'can', $event->getTransition()->getName());
    ...
    public function onPreTransition(TransitionEvent $event)
    {
        $object = $event->getStateMachine()->getObject();
        $previousState = $event->getStateMachine()->getCurrentState();
        $this->callCallback($object, $previousState, 'pre', $event->getTransition()->getName());
    }
    ...
    public function onPostTransition(TransitionEvent $event)
    {
        $object = $event->getStateMachine()->getObject();
        $previousState = $event->getStateMachine()->getCurrentState();
        $this->callCallback($object, $previousState, 'post', $event->getTransition()->getName());
    }
     ...
     * @return mixed
     */
    protected function callCallback($object, $previousState, $callbackPrefix, $transitionName)
    {
        ...
        return call_user_func(array($this, $methodName), $object, $previousState);
    }
}
